...
<div class="modal-body">
    <form class="form-horizontal form" role="form" id="updateModel" action="/node/update">
                ...
                <select class="form-control" id="workId" >
                    ...
                    <#list allEnableWorkList as item>
                        <option value="${item.workId}"
                                <#if model?? && model.workId?? && model.workId == item.workId>selected</#if>>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-10">
                <textarea class="textarea form-control" name="nodeDesc" placeholder="${I18n.system_please_input}" maxlength="512" style="height: 63px; line-height: 1.2;">${(model.nodeDesc)!}</textarea>
            </div>
        </div>
        <div class="form-group">
                ...
                <select class="form-control" id="nodeStatus" name="nodeStatus">
                    ...
                    <#list workStatusEnum as item>
                        <option value="${item.code}"
                                <#if model?? && model.nodeStatus?? && model.nodeStatus == item.code>selected</#if>>
                            ${item.value}
                        </option>
                    </#list>
                </select>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-4">
                <input type="text" class="form-control" name="executeHandler" placeholder="${I18n.system_please_input}"
                       maxlength="50" value="${(model.executeHandler)!}">
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-sm-4">
                <select class="form-control" id="scriptType" name="scriptType">
                    ...
                    <#list scriptTypeEnum as item>
                        <option value="${item.code}"
                                <#if model?? && model.scriptType?? && model.scriptType == item.code>selected</#if>>
                            ${item.code}
                        </option>
                    </#list>
                </select>
